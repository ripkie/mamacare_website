  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-brand-gray-border overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-brand-gray-border bg-brand-gray-soft">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="p-2 rounded-xl hover:bg-brand-gray-border transition-colors"
            >
        ...
        <div className="px-5 py-4 grid grid-cols-2 gap-3">
          <div>
            ...
            <p className="font-mono text-xs text-brand-navy/70 mt-0.5 break-all">
              {patient.patientId}
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-sm font-medium text-brand-navy/70 mt-0.5">
              {createdAt
                ? format(createdAt, 'dd MMM yyyy, HH:mm', { locale: localeId })
                : '—'}
            </p>
          </div>
        </div>
      ...
      <LatestMeasurementCard
        measurement={patient.latestMeasurement ?? null}
        rot={patient.latestROT ?? null}
      />
